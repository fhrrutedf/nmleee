...
export default function EditCoursePage() {
    ...
    return (
        <div className="max-w-6xl mx-auto pb-40 px-4 text-right" dir="rtl">
            ...
            {/* --- TOP HUB HEADER --- */}
            <div className="mb-8 bg-white rounded-[2.5rem] p-8 shadow-premium border border-slate-50 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-black text-slate-900 leading-tight line-clamp-1">{formData.title}</h1>
                            <div className="flex items-center gap-3 mt-2">
                                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${formData.isActive ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                ...
                <div className="flex items-center gap-3">
                    <button 
                        ...
                        className="flex-1 lg:flex-none px-6 py-3.5 bg-slate-900 text-white rounded-2xl font-black text-xs flex items-center justify-center gap-2 shadow-xl hover:bg-black transition-all"
                    >
                        <FiEye /> معاينة الكورس
                    </button>
                    <button 
                        onClick={() => handleSave()}
                        disabled={saving}
                        className="flex-1 lg:flex-none px-8 py-3.5 bg-primary-indigo-600 text-white rounded-2xl font-black text-xs flex items-center justify-center gap-2 shadow-xl shadow-primary-indigo-100 hover:bg-primary-indigo-700 transition-all disabled:opacity-50"
                    >
            ...
            <div className="flex p-1.5 bg-slate-100 rounded-[1.5rem] mb-10 w-full lg:w-fit overflow-x-auto no-scrollbar">
                {[
                    ...
                    <button
                        ...
                        className={`flex shrink-0 items-center gap-2 px-6 py-3 rounded-2xl text-xs font-black transition-all ${activeTab === tab.id ? 'bg-white text-primary-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        {tab.icon} {tab.label}
                    </button>
                ))}
                ...
                >
                        ...
                        <div className="grid lg:grid-cols-5 gap-10">
                            ...
                            <div className="lg:col-span-3 space-y-10">
                                <HubSection title="بيانات الدورة والمعلومات" icon={<FiPackage />}>
                                    <div className="space-y-6">
                                        <div>
                                            <label className="text-xs font-black text-slate-500 mb-2 block">عنوان الدورة</label>
                                            <input
                                        ...
                                        <div>
                                            <label className="text-xs font-black text-slate-500 mb-2 block">وصف الدورة التدريبية</label>
                            ...
                            <div className="lg:col-span-2 space-y-10">
                                <HubSection title="إعدادات السعر والعملة" icon={<FiBarChart2 />}>
                                    <div className="space-y-6">
                                        <div>
                                            <label className="text-xs font-black text-slate-500 mb-2 block text-center">العملة المختارة</label>
                                            <div className="flex gap-2 p-1 bg-slate-50 rounded-xl">
                                                <button 
                                                    type="button" onClick={() => setFormData({ ...formData, currency: 'USD' })}
                                                    className={`flex-1 py-2 rounded-lg text-[10px] font-black transition-all ${formData.currency === 'USD' ? 'bg-primary-indigo-600 text-white shadow-md' : 'text-slate-400'}`}
                                                >
                                                    دولار (USD)
                                                </button>
                                                <button 
                                                    type="button" onClick={() => setFormData({ ...formData, currency: 'SYP' })}
                                                    className={`flex-1 py-2 rounded-lg text-[10px] font-black transition-all ${formData.currency === 'SYP' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400'}`}
                                                >
                                                    ليرة سورية (SYP)
                                                </button>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4 text-center">
                                            <div>
                                                <label className="text-[10px] font-black text-slate-400 mb-1 block">السعر النهائي</label>
                                                <input
                                                    type="number" className="input-modern text-center font-black text-lg border-2 border-primary-indigo-50 text-primary-indigo-600"
                                                    value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })}
                                                />
                                            </div>
                                            <div>
                                                <label className="text-[10px] font-black text-slate-400 mb-1 block italic">السعر السابق</label>
                                                <input
                                ...
                                <HubSection title="صورة الغلاف" icon={<FiImage />}>
                                    <div className="space-y-6">
                                        <div className="aspect-video relative rounded-[2rem] overflow-hidden border-4 border-white shadow-xl bg-slate-50 group">
                                            {formData.image ? (
                                                <>
                    ...
                    {activeTab === 'curriculum' && (
                        <div className="space-y-8">
                             <div className="flex justify-between items-center bg-white rounded-3xl p-6 border border-slate-50 shadow-sm">
                                <div>
                                    <h2 className="text-xl font-black text-slate-900 pr-4 border-r-4 border-primary-indigo-600 leading-none">محتويات الدورة</h2>
                                    <p className="text-[10px] text-slate-400 font-black mt-2 pr-4">قم ببناء الفصول وإضافة الدروس والفيديوهات</p>
                                </div>
                                <button 
                                    onClick={() => router.push(`/dashboard/courses/${courseId}/content`)}
                                    className="px-6 py-3 bg-primary-indigo-50 text-primary-indigo-600 rounded-2xl font-black text-xs hover:bg-primary-indigo-100 transition-all flex items-center gap-2 border border-primary-indigo-100 shadow-sm"
                                >
                             ...
                             {modules.length === 0 ? (
                                 <div className="text-center py-20 bg-white rounded-[3.5rem] border-2 border-dashed border-slate-100 mt-6">
                                     ...
                                     <p className="text-slate-300 font-black">المنهج فارغ حالياً</p>
                                     <button onClick={() => router.push(`/dashboard/courses/${courseId}/content`)} className="mt-6 px-8 py-3 bg-slate-900 text-white rounded-xl text-xs font-black">ابدأ الآن</button>
                                 </div>
                             ) : (
                                 <div className="grid gap-6">
                                     {modules.map((m: any, idx: number) => (
                                         <div key={m.id} className="bg-white rounded-[2.5rem] border border-slate-50 shadow-premium overflow-hidden group">
                                             <div className="p-6 bg-slate-50/30 border-b border-slate-50 flex items-center justify-between">
                                                 <div className="flex items-center gap-4">
                                                     <div className="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-xs font-black text-slate-400 shadow-sm">{idx + 1}</div>
                                                     <div>
                                                         <h3 className="font-black text-slate-800 text-sm">{m.title}</h3>
                                                         <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{m.lessons?.length || 0} درساً</span>
                                                     </div>
                                                 </div>
                                                 <button 
                                                    onClick={() => router.push(`/dashboard/courses/${courseId}/modules/${m.id}/lessons/new`)}
                                                    className="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-emerald-500 hover:border-emerald-100 hover:bg-emerald-50 transition-all"
                                                 >
                                                    <FiPlus size={14} />
                                                 </button>
                                             </div>
                                                 ...
                                                 {(m.lessons || []).map((l: any, lIdx: number) => (
                                                     <div key={l.id} className="p-4 px-8 flex items-center justify-between hover:bg-slate-50/20 transition-colors">
                                                         <div className="flex items-center gap-4 min-w-0">
                                                             <span className="text-[10px] font-black text-slate-300">{lIdx + 1}</span>
                                                             <div className="truncate">
                                                                 <h4 className="text-xs font-black text-slate-700 truncate">{l.title}</h4>
                                                                 <div className="flex items-center gap-2 mt-1 opacity-60">
                                                                    {l.isFree && <span className="text-[8px] bg-amber-50 text-amber-600 px-1.5 py-0.5 rounded font-black italic">معاينة</span>}
                                                                    {l.bunnyVideoId ? <FiVideo size={10} className="text-primary-indigo-400" /> : <FiFileText size={10} />}
                                                                    {l.isPublished ? <FiEye className="text-emerald-500" size={10} /> : <FiEyeOff size={10} />}
                                                                 </div>
                                                             </div>
                                                         </div>
                                                         <button 
                    ...
                    {activeTab === 'students' && (
                        <div className="bg-white rounded-[3.5rem] p-16 text-center border-2 border-dashed border-slate-100 shadow-sm">
                             ...
                             <h3 className="text-xl font-black text-slate-900 mb-2">إدارة الطلاب والمشتركين</h3>
            ...
            <div className="fixed bottom-8 left-0 right-0 z-50 px-4">
                <div className="max-w-4xl mx-auto bg-slate-900/95 backdrop-blur-xl border border-white/10 p-4 rounded-[2.5rem] shadow-2xl flex items-center justify-between">
                    <div className="pr-6 hidden sm:block">
                        <p className="text-[10px] text-white/40 font-black uppercase tracking-[0.2em] leading-tight">اسم الكورس المختار</p>
                        <p className="text-xs text-white font-black italic mt-1 truncate max-w-[200px]">{formData.title || 'بدون عنوان'}</p>
                    </div>
                    <div className="flex gap-4">
                        <Link href="/dashboard/courses" className="px-8 py-3.5 text-white/50 hover:text-white font-black text-xs transition-all">إلغاء</Link>
                        <button 
                            onClick={() => handleSave()}
                            disabled={saving}
                            className="px-12 py-3.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-2xl font-black text-xs shadow-lg shadow-emerald-500/20 hover:scale-[1.03] active:scale-95 transition-all disabled:opacity-50"
                        >
...
function HubSection({ title, icon, children }: any) {
    return (
        <div className="bg-white rounded-[2.5rem] p-8 lg:p-10 shadow-premium border border-slate-50 space-y-8 animate-in fade-in duration-500">
            <div className="flex items-center gap-4 border-b border-slate-50 pb-6 -mx-2">
                ...
                <div className="text-right">
                    ...
                    <p className="text-[10px] text-slate-400 font-black mt-2 uppercase tracking-widest">إدارة المحتوى</p>
                </div>
            </div>
            <div>
                {children}
