...
export default function LearnPage() {
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-gray-950">
                <div className="relative w-20 h-20">
                    <div className="absolute inset-0 border-4 border-emerald-600-500/20 rounded-xl"></div>
                    <div className="absolute inset-0 border-4 border-t-accent-500 rounded-xl animate-spin"></div>
                </div>
        ...
        return (
            <div className="min-h-screen bg-gray-950 flex items-center justify-center p-6 text-right" dir="rtl">
                <div className="max-w-md w-full p-10 bg-gray-900 border border-white/5 rounded-xl shadow-lg shadow-[#10B981]/20">
                    ...
                    <button onClick={() => router.push('/courses')} className="w-full py-4 bg-emerald-700 text-white-600 hover:bg-emerald-700 text-white-500 text-white rounded-xl font-bold transition-all flex items-center justify-center gap-3">
    ...
    return (
        <div className="min-h-screen bg-gray-950 flex flex-col h-screen overflow-hidden text-right selection:bg-emerald-700 text-white-500/30" dir="rtl">
            ...
            <header className="h-16 md:h-20 bg-gray-900/80  border-b border-white/5 px-4 md:px-8 flex items-center justify-between z-30 shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div className="hidden md:flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-emerald-700 text-white-600 flex items-center justify-center text-white">
                            <FiAward size={16} />
                        </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="hidden sm:flex flex-col items-end gap-1.5 ml-4">
                        ...
                        <div className="w-32 h-1.5 bg-white/5 rounded-xl overflow-hidden">
                            <div className="h-full bg-emerald-700 text-white-500 rounded-xl transition-all duration-1000" style={{ width: `${progressPercent}%` }} />
                        </div>
                    </div>
                    <button 
            ...
            <div className="flex-1 flex overflow-hidden relative">
                                                ...
                                                {(module.lessons || []).map((l: any, idx: number) => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`w-full text-right p-3.5 rounded-xl flex items-center gap-3 transition-all ${isActive ? 'bg-emerald-700 text-white-600 text-white shadow-lg shadow-[#10B981]/20 shadow-accent-600/20' : 'hover:bg-white/5 text-gray-400'}`}
                                                        >
                                                            <div className={`w-6 h-6 rounded-lg shrink-0 flex items-center justify-center text-[10px] font-bold border transition-colors ${l.completed ? 'bg-emerald-700 text-white-500 border-emerald-600-500 text-white' : isActive ? 'bg-white/20 border-transparent text-white' : 'border-white/10 text-gray-500'}`}>
                            ...
                            <div className="space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
                                ...
                                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                                    ...
                                    <button 
                                        ...
                                        className={`flex items-center justify-center gap-2 h-12 px-6 rounded-xl font-bold text-xs transition-all ${showComments ? 'bg-emerald-700 text-white-600 text-white' : 'bg-white/5 text-gray-400 hover:bg-[#0A0A0A]/10'}`}
                                    >
                                ...
                                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                                    ...
                                    <button 
                                        ...
                                        className={`w-full sm:w-auto px-10 h-14 rounded-xl font-bold flex items-center justify-center gap-3 transition-all ${lesson?.completed ? 'bg-emerald-700 text-white-500 text-white' : 'bg-emerald-700 text-white-500/10 text-[#10B981]-500 border border-emerald-600-500/20 hover:bg-emerald-700 text-white-500 hover:text-white'}`}
                                    >
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.3em]">عن هذا الدرس</span>
                                        <div className="h-px bg-white/5 flex-1"></div>
                                    </div>
                                    <div className="bg-white/[0.02] border border-white/5 p-6 md:p-10 rounded-xl text-gray-300 leading-relaxed font-medium">
                                        {activeItem.data.content ? (
                                            ...
                                            <p className="text-center text-gray-400 py-10">لا يوجد وصف إضافي لهذا الدرس.</p>
                                        )}
                                    </div>
                                </div>
                ...
                <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-gray-900/80  border border-white/10 px-4 py-3 rounded-xl shadow-lg shadow-[#10B981]/20 z-40">
                    ...
                    >
                        ...
                        <div className="w-5 h-5 bg-emerald-700 text-white-600 rounded-xl flex items-center justify-center text-[8px]">{completedCount}/{totalLessons}</div>
                    </button>
                    <div className="w-px h-6 bg-[#0A0A0A]/10"></div>
